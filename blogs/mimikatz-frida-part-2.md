...
The first global variable we're interested in is *lsasrv!LogonSessionList*. As the name implies, this is a linked list where every element represents a single credential cached in memory. The way Mimikatz hunts for it is clever; instead of trying to identify the variable itself in memory, it looks for an instruction in *lsasrv.dll* which dereferences the address of the variable. This instruction will look something like this:
...
This is where the signature comes in. The signatures used by Mimikatz were presumably originally generated by reverse engineering; simply attach a debugger to *lsass.exe*, find instructions that dereference the variables we're interested in, and find some sequence of bytes nearby to be your "signature". When we identify this signature in memory at a given address, we can add a certain offset to it and get to the instruction in question. The exact process looks like this:
