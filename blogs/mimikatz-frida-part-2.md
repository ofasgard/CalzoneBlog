...
That's an intimidating struct! Don't worry, we won't need all of it. This is just the first element in a linked list, representing a single cached credential. We want all of the credentials, so the first order of business is to enumerate the address of every other element in the list. The *Flink* variable, a pointer to the next element in the list, is the very first element, so this is an easy job:
